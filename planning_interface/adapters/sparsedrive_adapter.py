"""SparseDrive 专用适配器：将 SparseDrive forward 输出映射到 PlanningInterface。

SparseDrive 输出字典的关键字段（从 motion_plan_head）:
...
注意:
- ego_fut_mode = 6（SparseDrive 有 6 个规划模式）
- ego_fut_ts = 6（6 个时间步）
...
class SparseDrivePlanningAdapter(BasePlanningAdapter):
    """SparseDrive → PlanningInterface 适配器。

    Args:
        scene_pool: 场景特征池化方式，'mean' / 'instance' / 'det_weighted'
        ego_fut_mode: 自车规划模式数，默认 6（SparseDrive）
        fut_ts: 未来时间步长数，默认 6
    """
    ...
    def extract_scene_token(
        ...
        """从 SparseDrive 输出中提取场景 token。

        SparseDrive 使用 sparse instance 而非密集 BEV，有多种提取方式:
        ...
        # 5. 默认：返回随机特征（仅用于调试）
        logger.warning('无法从 SparseDrive 输出中提取 scene_token，使用随机特征')
    ...
    def extract_reference_plan(
        ...
        SparseDrive planning_output 格式:
        ...
        if 'planning_output' in planner_outputs and planner_outputs['planning_output'] is not None:
            ...
            if 'prediction' not in plan_out:
                raise KeyError('SparseDrive planning_output 中缺少 prediction')
        ...
        raise KeyError('无法从 SparseDrive 输出中提取 reference_plan')
