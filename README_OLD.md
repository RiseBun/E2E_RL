...
- [运行示例 (SparseDrive)](#基于sparsedrive-的运行示例)
- [项目结构](#项目结构)
- [集成新模型](#集成新模型)
- [坐标系约定](#坐标系约定)
- [实验结果](#实验结果)
...
> **⚠️ 重要**: SparseDrive 与 VAD 框架相同（mmdet3d），可以共用 conda 环境

### Step 0: 环境配置

```bash
# SparseDrive 与 VAD 共享环境
conda activate vad  # 或使用您的 VAD 环境
...
# 安装 SparseDrive 依赖
cd projects/SparseDrive
pip install -r requirement.txt
cd ..
```

### Step 1: 放置 SparseDrive 项目
...
# 或复制已有的 SparseDrive 项目
cp -r /path/to/SparseDrive .
...
期望输出（ego-centric 坐标系）：
```
GT终点距原点: tensor([16.5, 18.2, 20.1])
Ref终点距原点: tensor([16.3, 18.0, 19.8])
```
...
> **⚠️ 重要**: 编辑 `scripts/train_evaluator_v2.py` 中的 CONFIG：
...
> **⚠️ 重要**: 编辑 `scripts/expC_relaxed.py` 中的 CONFIG：
