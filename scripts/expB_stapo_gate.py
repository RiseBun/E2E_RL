"""
实验 B: SafetyGuard + STAPOGate (规则基线)
...
CONFIG = {
    ...
    'output_dir': '/mnt/cpfs/prediction/lipeinan/RL/E2E_RL/experiments/ab_comparison/expB_stapo_gate',
}
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("防御层级配置 (实验 B: SafetyGuard + STAPOGate - 规则基线)")
    ...
    if metrics['rl_metrics']:
        for i, m in enumerate(metrics['rl_metrics']):
            logger.info(
                ...
                f"stapo_retention={m['stapo_retention']:.2%}"
            )

        final_rl = metrics['rl_metrics'][-1]
        logger.info(f"\n最终 retention ratio: {final_rl['retention_ratio']:.2%}")
        logger.info(f"STAPO gate retention: {final_rl['stapo_retention']:.2%}")
