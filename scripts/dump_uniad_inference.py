"""导出真实 VAD 推理输出 + GT + PlanningInterface 到磁盘。

用途:
    将 VAD 模型在 val set 上的推理结果逐帧保存，供后续离线分析使用：
    - confidence 有效性分析（真实数据）
    - A/B 对比（mean vs grid pooling）
    - low-confidence / hard subset 分析

实现方式:
    1. 复用 tools/test.py 的模型/数据加载逻辑
...
使用方式:
    python E2E_RL/scripts/dump_vad_inference.py \
