...
CONFIG = {
    ...
    'output_dir': '/mnt/cpfs/prediction/lipeinan/RL/E2E_RL/experiments/ab_comparison_v2/expA_safety_guard_only',
}
...
def main():
    ...
    safety_guard = SafetyGuard(
        SafetyGuardConfig(
            enabled=True,
            max_residual_norm=50.0,  # 放宽 10x
            max_step_disp=20.0,       # 放宽 10x
            max_speed=50.0,           # 放宽
            max_total_disp=100.0,     # 放宽 10x
            dt=0.5,
        )
    )
    ...
    logger.info("=" * 60)
    logger.info("防御层级配置 (实验 A: SafetyGuard Only - 放宽约束)")
